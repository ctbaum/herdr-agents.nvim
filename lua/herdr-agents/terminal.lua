...
function M.provider()
  ...
  function provider.open(command, env, _, should_focus)
    ...
    job = vim.fn.termopen(argv, { env = vim.tbl_isempty(env or {}) and vim.empty_dict() or env, cwd = vim.fn.getcwd() })
    if job <= 0 then
  ...
  function provider.submit(payload, opts)
    if not provider.paste("\27[200~" .. payload .. "\27[201~", opts) then return false end
